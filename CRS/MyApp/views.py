...
def register(request):
    if request.method == "POST":
        ...
        myuser.name = name
        myuser.save()
        html_content=render_to_string("email_template.html",{"name":name,'username':username,'number':number,'email':email,'password':password})
...
def order(request):
    if request.method == "POST":
        ...
        html_content=render_to_string("email_template2.html",{"billname":billname,'billemail':billemail,'billphone':billphone,'billaddress':billaddress,'billcity':billcity,'cars11':cars11,'dayss':dayss,'date':date})
        text_content=strip_tags(html_content)
